...
- `Bambu STEP` и `Bambu STL` запускают `bambu-studio.exe <file>` без дополнительных флагов; Bambu Studio сам переиспользует открытое окно, а если окна нет, запускается;
